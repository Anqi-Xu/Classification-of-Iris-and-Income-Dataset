...
def testing(myTree, classList, data_test, labels, labelProperties):
    ...
    for i in range(len(data_test)):
        classLabelSet = classify(myTree, classList, labels, labelProperties, data_test[i])  # classlabel
        maxWeight = 0.0
        classLabel = ''
...
# 测试决策树正确率
def acctesting(myTree, classList, data_test, labels, labelProperties):
    error = 0.0
    acc=0
    for i in range(len(data_test)):
        classkey = data_test[i][-1]
        classLabelSet = classify(myTree, classList, labels, labelProperties, data_test[i])  # classlabel
        #maxWeight = 0.0
        #classLabel = ''
        if classLabelSet[classkey] != 0:
            acc += 1
